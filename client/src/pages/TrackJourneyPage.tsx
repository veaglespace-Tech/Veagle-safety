...
import { Navigation, Clock, ShieldCheck, CheckCircle, ArrowRight, Info } from 'lucide-react';

export const TrackJourneyPage: React.FC = () => {
  const { latitude, longitude } = useLocationStore();
  const [activeTab, setActiveTab] = useState<'JOURNEY' | 'CHECKIN'>('JOURNEY');
  ...
  const loadActiveJourney = async () => {
    try { const res = await api.get('/journey/active'); setJourney(res.data.journey); } catch {}
  };
  const loadActiveCheckin = async () => {
    try { const res = await api.get('/checkin/active'); setCheckin(res.data.checkin); } catch {}
  };
  ...
  const handleStartJourney = async (e: React.FormEvent) => {
    ...
    } catch { alert('Failed to start journey. Please try again.'); }
    finally { setLoading(false); }
  };

  const handleCompleteJourney = async () => {
    if (!journey) return;
    try { await api.post('/journey/complete', { journeyId: journey.id }); setJourney(null); } catch {}
  };
  ...
  const handleConfirmSafeCheckin = async () => {
    if (!checkin) return;
    try { await api.post('/checkin/safe', { checkinId: checkin.id }); setCheckin(null); } catch {}
  };
