...
export const SuperAdminQuickJump = () => {
  ...
  const targetLabel = isAdminPage ? 'Switch to Website' : 'Admin HQ';

  return (
    <div className="fixed bottom-5 right-4 sm:bottom-6 sm:right-6 z-[9999] font-sans">
      {/* DIRECT 1-TAP FLOATING 3D SWITCHER BUTTON */}
      <Link
        href={targetHref}
      ...
      >
        ...
        <div className="flex items-center space-x-1.5 text-left">
          ...
          <ArrowRight className="w-3.5 h-3.5 text-[#FF2A6D] group-hover:translate-x-1 transition-transform" />
        </div>
      </Link>
    </div>
  );
