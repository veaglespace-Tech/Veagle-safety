...
export const SOSHeroButton: React.FC<SOSHeroButtonProps> = ({ onTriggerComplete }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center my-6">
      <div className="relative flex items-center justify-center">
        <div
          className={`absolute inset-0 rounded-full bg-tichi-emergency/15 ${
            holding ? 'animate-none scale-125 bg-tichi-emergency/30' : 'animate-radar-pulse'
          }`}
      ...
      <div className="mt-6 flex flex-col items-center space-y-2">
        <p className="text-xs text-tichi-muted font-medium text-center">
          Press & hold for 3 seconds to instantly alert contacts
        </p>

        <button
          onClick={() => setIsSilent(!isSilent)}
          className={`flex items-center space-x-2 text-xs font-semibold px-3.5 py-1.5 rounded-full transition-all border ${
            isSilent
              ? 'bg-plum text-rose border-plum'
              : 'bg-white text-tichi-muted border-blush-border hover:bg-plum-50'
          }`}
        >
          <span>Silent SOS: {isSilent ? 'ON (No Alarm Siren)' : 'OFF'}</span>
        </button>
      </div>
    </div>
  );
