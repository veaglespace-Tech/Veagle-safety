          ...
          <div className="bg-white p-6 sm:p-8 rounded-[36px] border-2 border-[#FFCCE1] shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-[#684E67]">Manage B2B partners, NGO affiliates, and custom referral links.</p>
              </div>
            </div>
            <button
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-[#FFF0F3]/60 border-b-2 border-[#FFCCE1]">
                    <th className="p-5 text-[11px] font-black text-[#2A0826] uppercase tracking-widest">Partner Name</th>
                    <th className="p-5 text-[11px] font-black text-[#2A0826] uppercase tracking-widest">Contact Info</th>
                    <th className="p-5 text-[11px] font-black text-[#2A0826] uppercase tracking-widest">Unique Code</th>
                    <th className="p-5 text-[11px] font-black text-[#2A0826] uppercase tracking-widest text-center">Referred Users</th>
                ...
                <tbody className="divide-y divide-[#FFCCE1]/50">
                  {isLoading ? (
                    <tr>
                      <td colSpan={6} className="p-8 text-center text-sm font-bold text-[#684E67]">Loading partners...</td>
                    </tr>
                  ) : partners.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-8 text-center text-sm font-bold text-[#684E67]">No referral partners found.</td>
                    </tr>
                  ) : (
                    partners.map(partner => (
                      <tr key={partner.id} className="hover:bg-[#FFF0F3]/30 transition-colors">
                        ...
                        <td className="p-5 space-y-1">
                          <p className="text-xs font-bold text-[#684E67] flex items-center space-x-1"><Mail className="w-3 h-3 text-[#FF2A6D]"/> <span>{partner.email}</span></p>
                          {partner.mobile && <p className="text-xs font-bold text-[#684E67] flex items-center space-x-1"><Phone className="w-3 h-3 text-[#FF2A6D]"/> <span>{partner.mobile}</span></p>}
                        </td>
                        <td className="p-5">
                        ...
                        <td className="p-5 text-center">
                          {partner.isActive ? (
                            ...
                            <span className="text-[10px] font-black text-gray-500 bg-gray-100 px-3 py-1 rounded-full uppercase">Inactive</span>
                          )}
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#2A0826]/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-[32px] border-2 border-[#FFCCE1] shadow-2xl overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black text-[#2A0826] uppercase tracking-wider">Email Address *</label>
                <input required type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} className="w-full px-4 py-3 bg-[#FFF0F3]/30 border border-[#FFCCE1] rounded-2xl text-sm font-bold outline-none focus:border-[#FF2A6D] focus:ring-2 focus:ring-[#FF2A6D]/20 transition-all" placeholder="partner@example.com" />
              </div>
              <div className="space-y-1.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#2A0826]/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[32px] border-2 border-[#FFCCE1] shadow-2xl flex flex-col">
            <div className="p-6 border-b border-[#FFCCE1] flex items-center justify-between bg-[#FFF0F3]/30 shrink-0">
              <div>
                <h3 className="font-black text-xl text-[#2A0826]">{selectedPartner.name} - Network Stats</h3>
                <p className="text-xs font-bold text-[#FF2A6D]">Referral Code: {selectedPartner.partnerReferralCode}</p>
              </div>
              <button onClick={() => setIsStatsModalOpen(false)} className="p-2 hover:bg-[#FFCCE1] rounded-full text-[#684E67] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto bg-gray-50/50">
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm text-center">
                  <p className="text-xs font-black text-[#684E67] uppercase tracking-wider">Total Referrals</p>
                  <p className="text-3xl font-black text-[#2A0826] mt-1">{selectedPartner.referredUsers?.length || 0}</p>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-emerald-200 shadow-sm text-center">
                  <p className="text-xs font-black text-emerald-700 uppercase tracking-wider">Active Subscribers</p>
                  <p className="text-3xl font-black text-emerald-600 mt-1">
                    {selectedPartner.referredUsers?.filter(u => u.subscriptionStatus === 'ACTIVE').length || 0}
                  </p>
                </div>
              </div>

              <h4 className="font-black text-sm text-[#2A0826] uppercase tracking-wider mb-3 pl-1">Referred Users Directory</h4>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#FFF0F3]/50 border-b border-[#FFCCE1]">
                      <th className="p-4 text-[10px] font-black text-[#2A0826] uppercase tracking-widest">User Name</th>
                      <th className="p-4 text-[10px] font-black text-[#2A0826] uppercase tracking-widest">Email</th>
                      <th className="p-4 text-[10px] font-black text-[#2A0826] uppercase tracking-widest text-center">Joined At</th>
                      <th className="p-4 text-[10px] font-black text-[#2A0826] uppercase tracking-widest text-right">Sub Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#FFCCE1]/30">
                    {selectedPartner.referredUsers?.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="p-6 text-center text-xs font-bold text-gray-500">No users referred yet.</td>
                      </tr>
                    ) : (
                        ...
                        <tr key={user.id} className="hover:bg-gray-50">
                          ...
                          <td className="p-4 text-right">
                            {user.subscriptionStatus === 'ACTIVE' ? (
                              <span className="bg-emerald-100 text-emerald-700 font-black px-2.5 py-1 rounded-lg text-[10px] uppercase">Active</span>
                            ) : (
                              <span className="bg-gray-100 text-gray-600 font-black px-2.5 py-1 rounded-lg text-[10px] uppercase">{user.subscriptionStatus}</span>
                            )}
                          </td>
                        </tr>
                      ))
