...
export default function AdminSettingsPage() {
  ...
  const fetchSettings = async () => {
    try {
      const res = await settingApi.fetchSettings(['SUPPORT_EMAIL', 'GALLERY_ITEMS', 'GALLERY_META'])
      if (res.success) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#FAFAFA] min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {/* Global Toast */}
        {toast && (
          <div className={`fixed top-6 right-6 z-50 px-6 py-3 rounded-xl font-bold text-sm shadow-xl flex items-center gap-3 animate-fade-in ${toast.type === 'error' ? 'bg-red-500 text-white' : 'bg-[#059669] text-white'
            }`}>
        ...
        <div className="bg-white p-6 md:p-8 rounded-[32px] border border-gray-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-[#2A0826] tracking-tight">Platform Configuration</h1>
              <p className="text-sm font-bold text-gray-500 mt-1">Manage global public-facing settings</p>
            </div>
          </div>
          ...
          <button
            ...
            className={`px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-wider flex items-center justify-center space-x-2 transition-all shadow-lg ${hasUnsavedChanges && !saving
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-72 shrink-0 space-y-2">
            <button
              ...
              className={`w-full text-left px-5 py-4 rounded-2xl font-black text-sm flex items-center space-x-3 transition-all ${activeTab === 'gallery'
            ...
            <button
              ...
              className={`w-full text-left px-5 py-4 rounded-2xl font-black text-sm flex items-center space-x-3 transition-all ${activeTab === 'general'
                ? 'bg-white text-[#FF2A6D] border border-[#FFCCE1] shadow-sm'
                : 'text-gray-500 hover:bg-gray-100'
                }`}
            >
            ...
            {activeTab === 'general' && (
              <div className="bg-white rounded-[32px] border border-gray-200 shadow-sm p-6 md:p-8 animate-fade-in space-y-8">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-bold text-gray-500">Core operational contact details</p>
                </div>
                ...
                <div className="max-w-xl space-y-4">
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs font-bold text-gray-400">This email is publicly displayed on the Contact Us page.</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="bg-white rounded-[32px] border border-gray-200 shadow-sm p-6 md:p-8 animate-fade-in space-y-10 min-h-[600px]">
                ...
                <div className="space-y-6">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-bold text-gray-500">Configure the public Gallery page headers and filter categories.</p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#FAFAFA] p-6 rounded-[28px] border border-gray-200">
                    <div className="space-y-2 md:col-span-2">
                      <label className="text-xs font-black text-gray-500 uppercase tracking-wider">Page Title</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <label className="text-xs font-black text-gray-500 uppercase tracking-wider">Page Subtitle</label>
                      <textarea
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <label className="text-xs font-black text-gray-500 uppercase tracking-wider">Filter Categories</label>
                      <input
                        type="text"
                      ...
                      />
                      <p className="text-[10px] font-bold text-gray-400">Comma-separated list of categories. Ensure 'ALL' is included if you want an 'All' filter.</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-gray-200 space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-bold text-gray-500">Add URLs to images/videos to display on the public Gallery page.</p>
                    </div>
                    <button
                  ...
                  <div className="space-y-6">
                    {galleryItems.length === 0 ? (
                      <div className="text-center py-20 bg-[#FAFAFA] rounded-[32px] border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-base font-black text-[#684E67]">No gallery items added yet.</p>
                        <p className="text-sm font-bold text-gray-400 mt-2">Click 'Add Media' to populate your showcase.</p>
                      </div>
                    ) : (
                            ...
                            <div className="flex flex-col xl:flex-row gap-8 pt-4 border-t border-gray-200">
                              ...
                              <div className="xl:w-1/3 shrink-0">
                                ...
                                <div className="w-full aspect-[4/3] bg-gray-100 rounded-2xl border border-gray-200 overflow-hidden flex items-center justify-center shadow-inner relative">
                                  {!item.mediaUrl ? (
                                    <span className="text-xs font-bold text-gray-400">No URL provided</span>
                                  ) : item.mediaType === 'video' ? (
                                    <video src={item.mediaUrl} className="w-full h-full object-cover" autoPlay loop muted playsInline />
                                  ) : (
                                    <img src={item.mediaUrl} alt="Preview" className="w-full h-full object-cover" onError={(e) => { e.target.src = ''; e.target.onerror = null; e.target.parentElement.innerHTML = '<span class="text-xs font-bold text-red-400">Broken Link</span>'; }} />
                                  )}

                                  {/* Preview Overlay info */}
                                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
                                    {item.badge && <span className="text-[10px] bg-[#FF2A6D] text-white px-2 py-0.5 rounded-full w-fit uppercase font-black mb-1">{item.badge}</span>}
                                    <span className="text-white font-black text-sm truncate">{item.title || 'Untitled'}</span>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-5">
                                ...
                                <div className="space-y-2 md:col-span-2">
                                  ...
                                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                                    <select
                                      ...
                                      onChange={(e) => updateGalleryItem(index, 'mediaType', e.target.value)}
                                    ...
                                    <input
                                      type="text"
                                      value={item.mediaUrl}
                                      onChange={(e) => updateGalleryItem(index, 'mediaUrl', e.target.value)}
                                ...
                                {/* Subtitle */}
                                <div className="space-y-2">
                                  <label className="text-xs font-black text-gray-500 uppercase tracking-wider">Subtitle</label>
                                  <input
                                    type="text"
                                    value={item.subtitle}
                                    onChange={(e) => updateGalleryItem(index, 'subtitle', e.target.value)}
                                    className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-[#FF5C8A]"
                                ...
                                {/* Badge */}
                                <div className="space-y-2">
                                  <label className="text-xs font-black text-gray-500 uppercase tracking-wider">Badge Text</label>
                                  <input
                                    type="text"
                                    value={item.badge}
                                    onChange={(e) => updateGalleryItem(index, 'badge', e.target.value)}
                                    className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-[#FF5C8A]"
                                    ...
                                    {(() => {
                                      ...
                                      if (galleryMeta?.categories) {
                                        if (Array.isArray(galleryMeta.categories)) {
                                          ...
                                          cats = galleryMeta.categories.split(',').map(c => c.trim()).filter(Boolean);
                                        }
                                      }
